...
public class Customer extends  Managedbean{
	...
	public String addAction() throws SQLException {
		try {
			...
			if (dbcustcurr.equals(c)) {
				...
				System.out.println("updated in SearchCustomer Table–" +query);
	 ...
	 public String  updateAction() throws SQLException {
		try {
			...
			for (int i = 0; i < custInfoAll.size(); i++) {
				...
				String query = "UPDATE revenue.manday SET mandayvalue = '"
				...
				String query1 = "UPDATE revenue.master SET startDate = '"+ dbcustda + "', endDate = '"+ dbcustdb + "' where    masterid ='" + dbcustmid + "' ";
				System.out.println("Update in Searchmaster Table–" + query1);
	...
	public List<Customer> getCustomerDetail() {
		...
		try {
			...
			String strSql = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database, t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,SUM(manday.mandayvalue) from revenue.master left join  revenue.manday on (master.masterid=manday.master_masterid) where customer_Name like '"+ searchName + "%' or br like '"+ searchName + "%' group by master.masterid ";
			...
			while (rs.next()) {
				...
				System.out.println(rs.getDouble("SUM(manday.mandayvalue)") + "manday");
				cust.setmandayvalue(rs.getDouble("SUM(manday.mandayvalue)"));
				System.out.println(rs.getDouble("SUM(manday.mandayvalue)") + "manday");
				cust.setdm(rs.getDouble("SUM(manday.mandayvalue)"));
		...
		try {
			...
			if (searchName2 == "" && datea == "" && dateb == "" && searchName3 == ""  && searchName4== "" ) {
				if (searchName1.equals(All) && datea == "" && dateb == "") {
					String strSql = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder, SUM(manday.mandayvalue)   from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid) where customer_Type in ('New', 'Existing','Inter Company') group by master.masterid  ";
					...
					String strSq = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,SUM(manday.mandayvalue) from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid) where  customer_Type='"+ searchName1 + "' group by master.masterid  ";
				...
				String strSq = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,SUM(manday.mandayvalue)   from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid) where  region='"
...
//vertical starts
				...
				String strSq = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,SUM(manday.mandayvalue)   from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid) where  vertical='"+ searchName3 + "' group by master.masterid  ";
				System.err.println("Search query:-" + strSq);
				rs = stmt.executeQuery(strSq);
				System.out.println(strSq);
				this.sum_up=0;
...
//region and service
				...
				String strSq = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,SUM(manday.mandayvalue)   from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid) where  service='"+ searchName4 + "' and region='"+ searchName2 + "' group by master.masterid  ";
				System.err.println("Search query:-" + strSq);
				rs = stmt.executeQuery(strSq);
				System.out.println(strSq);
				this.sum_up=0;
...
//custype & vertical & service
			...
			//
				...
				if (searchName1.equals(All) && datea == "" && dateb == "") {
					...
					String strSq = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,SUM(manday.mandayvalue) from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid) where  customer_Type='"
							+ searchName1
							+ "' and region= '"
							+ searchName2
...
//date starts // method for start and end date for report generation
				...
				String strSq = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,manday.mandayvalue,( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ),ROUND((updated_Work_Order/( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ) *(manday.mandayvalue)),2 ) from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid)    where  datec  BETWEEN '" +datea + "' AND '" +dateb + "' and mandayvalue IS NOT NULL and customer_Type='"+ searchName1+ "' and region= '"+ searchName2+ "' ";   
				...
				while (rs.next()) {
					...
					System.out.println(rs.getDouble(37) + "delmanday");
					...
					System.out.println(rs.getString("customer_Type")
							+ "custtypename");
					cust.setcustomer_Type(rs.getString("customer_Type"));
					System.out.println(rs.getString("region") + "region");
					cust.setRegion(rs.getString("region"));
					System.out.println(rs.getString("workOrder")
							+ "workOrder");
					cust.setWorkOrder(rs.getString("workOrder"));
					...
					cust.setindustry(rs.getString("industry"));
					System.out
							.println(rs.getString("service") + "service");
					cust.setservice(rs.getString("service"));
					System.out
							.println(rs.getString("product") + "product");
					cust.setproduct(rs.getString("product"));
					System.out.println(rs.getString("stage") + "stage");
					cust.setstage(rs.getString("stage"));
					System.out.println(rs.getString("designation")
				...
				String strSq11 = "select masterid,customer_Name,updated_Work_Order,( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ),ROUND((updated_Work_Order/( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ) *(manday.mandayvalue)),2) from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid)   where datec  BETWEEN '" +datea + "' AND '" +dateb + "' and mandayvalue IS NOT NULL   and customer_Type='"+ searchName1+ "' and region= '"+ searchName2+ "' group by masterid ";   
			...
			{	{
					...
					String strSq = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,manday.mandayvalue,( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ),ROUND((updated_Work_Order/( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ) *(manday.mandayvalue)),2 ) from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid)    where datec  BETWEEN '" +datea + "' AND '" +dateb + "' and mandayvalue IS NOT NULL  ";   
					...
					{
						String strSq11 = "select masterid,customer_Name,updated_Work_Order,( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ),ROUND((updated_Work_Order/( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ) *(manday.mandayvalue)),2)  from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid)    where datec  BETWEEN '" +datea + "' AND '" +dateb + "' and mandayvalue IS NOT NULL group by masterid ";   
				...
				String strSq = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,manday.mandayvalue,( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ),ROUND((updated_Work_Order/( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ) *(manday.mandayvalue)),2 ) from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid)    where datec  BETWEEN '" +datea + "' AND '" +dateb + "' and mandayvalue IS NOT NULL and  region= '"+ searchName2+ "' ";   
				...
				String strSq11 = "select masterid,customer_Name,updated_Work_Order,( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ),(updated_Work_Order/( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ) *(manday.mandayvalue)) from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid)    where datec  BETWEEN '" +datea + "' AND '" +dateb + "' and mandayvalue IS NOT NULL and   region= '"+ searchName2+ "'  group by masterid ";   
			...
			// last
				...
				if (searchName1.equals(All) && datea != "" && dateb != "") {
					String strSq = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,manday.mandayvalue,( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ),ROUND((updated_Work_Order/( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ) *(manday.mandayvalue)),2 ) from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid)    where datec  BETWEEN '" +datea + "' AND '" +dateb + "' and mandayvalue IS NOT NULL and  customer_Type in ('New', 'Existing','Inter Company') ";   
	...
	////date & vertical
				...
				String strSq = "select masterid,bR,customer_Name,customer_Type,region,vertical,industry,service,product,stage,designation,order_Booked_By,work_Order_Date,currency,work_Order_Assessment_fee,work_Order_Logo_fee,shared_Revenue,work_Order_Value,exchange_Rate,ROUND(updated_Work_Order,2),ROUND(updated_WO_Assessment_fee,2),work_Order_Mandays,start_month,delivered_Mandays,startDate,endDate,invoice,invoice_Date,assessment_Fees,logo_Fee_Database,t_L,ROUND(service_Tax,2),ROUND(total_Invoice_Amount,2),period_Mandays,workOrder,manday.mandayvalue,( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ),ROUND((updated_Work_Order/( select sum(manday.mandayvalue) from revenue.manday where master.masterid=manday.master_masterid  group by master.masterid ) *(manday.mandayvalue)),2 ) from revenue.master  left join  revenue.manday on (master.masterid=manday.master_masterid)    where  datec  BETWEEN '" +datea + "' AND '" +dateb + "' and mandayvalue IS NOT NULL and vertical='"+ searchName3+ "'  ";   
	...
	public List<Customer> getEntryDetail() {
		...
		try {
			...
			String sql1 = "Select masterid from revenue.master where customer_Name ='"+ customer_Name + "'   ";
			...
			String strSql = "select  date_ID,datec,mandayvalue from revenue.manday where datec  BETWEEN '"
					+ datea
					+ "' AND '"
					+ dateb
					+ "' and master_masterid='"
	...
	try {
	 ...
	 public List<Customer> getAuditDetail() {
			...
			try {
				...
				String strSql = "select  sample_ID,samplemanday.master_masterid,samplemanday.mandayvalue,sampletable.auditor_Name,table.datec,table.date_ID from  revenue.manday left join  revenue.sampletable on (manday.master_masterid=samplemanday.master_masterid)  where datec  BETWEEN '"+ datea+ "' AND '"+ dateb+ "' and auditor_Name='"+ searchName1+ "' and samplemanday.master_masterid='"+ masterid + "'  ";
				System.err.println("Search5 query:-" + strSql);
				rs = stmt.executeQuery(strSql);
